...
template<typename _DataType>
inline Node<_DataType>::Node()
	: parent(nullptr), childNodes(), data()
{
}
...
public:
	...
	void printLevelOrder();		// prints the level order from the position of the root node
	//enables prettier calls, eg: tree.printLevelOrder();

	NodeType * createNewNode(NodeType * dstNode);	/// creates a new node at destination with default values returning the newly created node

private:
	void clear(NodeType * node);	// clears the subtree from the position of the passed node, used by destructor
	NodeType * copyTree(NodeType * parent, NodeType * other);	// copies the tree on the right to the left, used by copy constructors
...
template <typename _DataType>
std::ostream& operator<<(std::ostream&, const Node<_DataType>& node);	// friend to ostream not needed because it has public access to struct anyways
...
template <typename _DataType>
inline bool
Tree<_DataType>::isEmpty() const {
	return !m_root;
}

template <typename _DataType>
inline Node<_DataType> *
Tree<_DataType>::getRoot() const {
	return m_root;
}

template <typename _DataType>
inline Node<_DataType> *
Tree<_DataType>::createNewNode(NodeType * dstNode) {
...
template <typename _DataType>
inline void
Tree<_DataType>::printLevelOrder(NodeType * rootNode) {
...
template<typename _DataType>
inline 
void Tree<_DataType>::printLevelOrder()
{
...
template <typename _DataType>
inline void
Tree<_DataType>::clear(NodeType * node) {
...
template <typename _DataType>
inline Node<_DataType> *
Tree<_DataType>::copyTree(NodeType * parent, NodeType * src) {
	if (src == nullptr) {
		return nullptr;
	}
...
template <typename _DataType>
inline std::ostream&
operator<<(std::ostream& os, const Node<_DataType>& node) {
	os << node.data;
